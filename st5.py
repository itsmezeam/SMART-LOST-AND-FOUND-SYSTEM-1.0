...
import json, uuid, secrets, hashlib, io   # ⬅️ add io here
...
# 🔗 change this to your real URL when deployed
APP_BASE_URL = "http://localhost:8501"

# Roles
ROLE_OWNER = "owner"      # item owner (lost item, claim)
ROLE_FINDER = "finder"    # item finder (deposits into locker)
ROLE_ADMIN = "admin"      # admin / staff
...
def ensure_fs():
    ...
    if not USERS_CSV.exists():
        pd.DataFrame(columns=[
            ...
            "reset_code","reset_expires_at","role"
        ]).to_csv(USERS_CSV, index=False)
    else:
        # 🔧 migrate existing users.csv to add "role" if missing
...
def create_user(email: str, name: str, password: str, role: str = ROLE_OWNER) -> Optional[dict]:
...
def token_valid(row: dict, provided: Optional[str] = None) -> bool:
    if not row or row.get("used") == "True":
        return False
    try:
        if provided is not None and (provided or "").strip() != row["token"]:
            return False
...
def render_qr_with_download(data: str, caption: str, filename: str):
    """
    Render QR in the app + provide a PNG download button.
    """
    img = make_qr_image(data)

    # Convert to PNG bytes once
    buf = io.BytesIO()
    ...
    st.image(byte_im, caption=caption, use_container_width=False)
...
def render_qr_with_download(data: str, caption: str, filename: str):
    """
    Render QR in the app + provide a PNG download button.
    `data` is the string to encode in the QR (usually a URL).
    """
    img = make_qr_image(data)
...
def page_register():
    ...
    with st.form("register_form"):
        ...
        role_label = st.selectbox(
            "Account type",
            ["Item Owner (lost item)", "Item Finder (deposits found items)"]
        )
    ...
    if ok:
        ...
        role_map = {
            "Item Owner (lost item)": ROLE_OWNER,
            "Item Finder (deposits found items)": ROLE_FINDER,
        }
        role = role_map[role_label]

        u = create_user(email, name, p1, role)
        if not u:
...
def page_home():
    ...
    role = user.get("role", ROLE_OWNER)

    c1, c2, c3, c4 = st.columns(4)

    if c1.button("➕ Report Lost Item"):
        st.session_state["nav_private_pending"] = "Report Lost Item"
        st.rerun()

    if c2.button("🔍 View Found Items"):
        st.session_state["nav_private_pending"] = "View Found Items"
        st.rerun()

    if role in (ROLE_FINDER, ROLE_ADMIN):
        if c3.button("⬇️ Deposit Item"):
            st.session_state["nav_private_pending"] = "Deposit Item"
            st.rerun()

    if role in (ROLE_OWNER, ROLE_ADMIN):
        if c4.button("✅ Claim Item"):
            st.session_state["nav_private_pending"] = "Claim Item"
            st.rerun()
...
def next_item_code() -> str:
    ...
    if df.empty:
        ...
        nums = []
        for v in df["id"]:
            if isinstance(v, str) and v.startswith("SLF_"):
                try:
                    nums.append(int(v.split("_")[1]))
                except ValueError:
                    pass
...
def page_deposit():
    ...
    if submit:
        ...
        # QR opens the app with item code in query params
        qr_url = f"{APP_BASE_URL}?item={item_id}&action=deposit"
        render_qr_with_download(
            data=qr_url,
            caption="Deposit QR (single-use, 30 min)",
            filename=f"deposit_{item_id}.png"
        )
...
def page_claim():
    ...
    if chosen["category"] == "unknown":
        ...
        if st.button("Submit claim & get pickup QR"):
            ...
            qr_url = f"{APP_BASE_URL}?item={item_id}&action=claim"
            render_qr_with_download(
                data=qr_url,
                caption="Pickup QR (single-use, 30 min)",
                filename=f"claim_{item_id}.png"
            )
    ...
    if st.button("Simulate unlock & claim"):
        ...
        if not tok_row or not token_valid(tok_row):  # just check expiry + used
            st.error("No active or expired claim token for this item.")
            return
...
def sidebar_nav():
    ...
    if user:
        ...
        # Role-based menu
        if role == ROLE_ADMIN:
            options = ["Home", "Deposit Item", "View Found Items", "Claim Item", "Report Lost Item", "Admin"]
        elif role == ROLE_FINDER:
            options = ["Home", "Deposit Item", "View Found Items", "Report Lost Item"]
        else:  # owner
            options = ["Home", "View Found Items", "Claim Item", "Report Lost Item"]

        # Check if a page was requested by a button (from Home page)
        pending = st.session_state.pop("nav_private_pending", None)

        # Decide what should be selected in the radio
        if pending in options:
            default = pending
        else:
            default = st.session_state.get("nav_private", options[0])

        # Convert default to radio index
        try:
            idx = options.index(default)
        except ValueError:
            idx = 0

        # IMPORTANT: we don't assign to st.session_state["nav_private"] manually;
        # letting the radio widget manage it.
        choice = st.sidebar.radio("Navigation", options, index=idx, key="nav_private")
...
def page_qr_view():
    st.header("Scanned Item Details")

    params = st.experimental_get_query_params()   # ✅ use this
    item_code = params.get("item", [""])[0]

    # Allow manual fallback (for testing)
    item_code = st.text_input("Item code (from QR)", value=item_code, placeholder="SLF_0001")

    if not item_code:
        st.info("Scan a QR code or enter an item code to view details.")
        return

    df = load_df(ITEMS_CSV)
    row = df[df["id"] == item_code]
    if row.empty:
        st.error(f"No item found for code: {item_code}")
        return

    r = row.iloc[0]
    st.subheader(f"Item: {r['name']}  ·  Locker {r['locker_id']}")

    c1, c2 = st.columns([1,2])
    if r["photo_path"] and Path(r["photo_path"]).exists():
        c1.image(Image.open(r["photo_path"]), caption="Item photo", use_container_width=True)

    c2.markdown(f"""
- **Item code:** `{r['id']}`
- **Found at:** {r['found_location'] or '-'}
- **Found time:** {r['found_datetime'] or '-'}
- **Status:** `{r['status']}`
- **Category:** `{r['category']}`
    """)

    # Show most recent token (if any)
    toks = load_df(TOKENS_CSV)
    trows = toks[toks["item_id"] == r["id"]]
    if not trows.empty:
        t = trows.sort_values("expires_at").iloc[-1]
        st.markdown("#### Token info")
        st.code(
            f"Token record ID: {t['id']}\n"
            f"Token value: {t['token']}\n"
            f"Action: {t['action']}\n"
            f"Expires at: {t['expires_at']}\n"
            f"Used: {t['used']}"
        )
    else:
        st.caption("No token records yet for this item.")


def router():
    # If coming from a QR scan with ?item=SLF_0001 in URL → show QR view page
    params = st.experimental_get_query_params()   # ✅ change this line
